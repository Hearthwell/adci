...
/* IF NULL GIVEN FOR DIMS, ALL DIMENSION ARE TO BE COUNTED */
struct adci_multi_dim_counter adci_tensor_init_multidim_counter(const struct adci_tensor *tensor, const unsigned int *dims, unsigned int length){
    ...
    if(dims == NULL){
        for(unsigned int i = 0; i < length; i++) counter.dim_indeces[i] = i;
    }else memcpy(counter.dim_indeces, dims, sizeof(unsigned int) * length);
